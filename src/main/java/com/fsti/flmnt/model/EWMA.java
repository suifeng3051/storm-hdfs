 /*****************************
 * Copyright (c) 2015 by Touch-Eline Co. Ltd.  All rights reserved.
 ****************************/
...
/** 
 * @{#} EWMA.java Create on 2015年1月18日 上午2:51:04
 * <p>
 * 	
 * </p>
 * @author <a href="mailto:cwx714@126.com">卫翔</a>   
 * @version v 0.1 
 */
public class EWMA implements Serializable {
	...
	public static enum Time {
		...
		MILLISECONDS(1), SECONDS(1000), MINUTES(SECONDS.getTime() * 60), 
		HOURS(MINUTES.getTime() * 60), DAYS(HOURS.getTime() * 24), WEEKS(DAYS.getTime() * 7);
	...
	public EWMA withAlpha(double alpha) {
		if(!(alpha >0.0D && alpha < 1.0D)) {
			throw new IllegalAccessError("alpha must be between 0.0 and 1.0");
		}
		this.alpha = alpha;
		return this;
	}
	...
	public EWMA withAlphaWindow(double count, Time time) {
		return this.withAlpha((long) (time.getTime() * count));
	}
	...
	public double getAveragePer(Time emitRatePer) {
		return this.average == 0.0 ? this.average : emitRatePer.getTime() / this.average;
	}
}
